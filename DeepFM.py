...
class DeepFM:
    ...
    def __init__(self, embed_dim, feat_dim_v1, feat_dim_v2, epoch, batch_size, hidden_dims):
    ...
    def build(self):
        ...
        with tf.name_scope('one_factor'):
            ...
            part1 = tf.reshape(tf.reduce_sum(w_x, axis=1), [-1, 1])  # batch*1
        ...
        with tf.name_scope('mutual_factor'):
            ...
            # 得到交互项的最终结果
            part2 = 1/2 * (part2_1 - part2_2)

        with tf.name_scope('deep_factor'):
            x_deep = tf.reshape(v_x, [-1, self.feat_dim_v1*self.embed_dim])
            for hidden_dim in self.hidden_dims:
                x_deep = tf.compat.v1.layers.dense(x_deep, hidden_dim, activation=tf.compat.v1.nn.relu)

        with tf.name_scope('output'):
            out = tf.concat([part1, part2, x_deep], axis=1)
            logits = tf.squeeze(tf.compat.v1.layers.dense(out, 1))
...
if __name__ == '__main__':
    ...
    model = DeepFM(5, loader.feat_dim_v1, loader.feat_dim_v2, 20, 1024, [32, 16])
    model.build()
